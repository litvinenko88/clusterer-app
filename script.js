document.addEventListener("DOMContentLoaded", function () {
  ...
  function removeGeoKeywords(keywordsData) {
    ...
    const geoPatterns = [
      // Города и их формы (москва, москве, москвой и т.д.)
      /(^|\s)(москв[а-яё]*|санкт-петербург[а-яё]*|спб|новосибирск[а-яё]*|екатеринбург[а-яё]*|казан[а-яё]*|нижний[\s-]?новгород[а-яё]*|челябинск[а-яё]*|самар[а-яё]*|омск[а-яё]*|ростов[а-яё]*|уф[а-яё]*|красноярск[а-яё]*|перм[а-яё]*|воронеж[а-яё]*|волгоград[а-яё]*|краснодар[а-яё]*|сочи[а-яё]*|владивосток[а-яё]*|иркутск[а-яё]*|хабаровск[а-яё]*|ярославл[а-яё]*|тюмен[а-яё]*|тольятти[а-яё]*|барнаул[а-яё]*|ижевск[а-яё]*|махачкал[а-яё]*|хасан[а-яё]*|оренбург[а-яё]*|новгород[а-яё]*|кемеров[а-яё]*|рязан[а-яё]*|астрахан[а-яё]*|набережные[\s-]?челны|пенз[а-яё]*|липецк[а-яё]*|киров[а-яё]*|чебоксар[а-яё]*|калининград[а-яё]*|тул[а-яё]*|ставропол[а-яё]*|белгород[а-яё]*|архангельск[а-яё]*|владимир[а-яё]*|севастопол[а-яё]*|курск[а-яё]*|улан-удэ|чебоксар[а-яё]*|калуг[а-яё]*|брянск[а-яё]*|курган[а-яё]*|орл[а-яё]*|владикавказ[а-яё]*|псков[а-яё]*|мурманск[а-яё]*|донецк[а-яё]*|луганск[а-яё]*|симферопол[а-яё]*)(\s|$)/i,

      // Области и регионы
      /(^|\s)(московск[а-яё]*\s*обл[а-яё]*|ленинградск[а-яё]*\s*обл[а-яё]*|свердловск[а-яё]*\s*обл[а-яё]*|нижегородск[а-яё]*\s*обл[а-яё]*|челябинск[а-яё]*\s*обл[а-яё]*|самарск[а-яё]*\s*обл[а-яё]*|омск[а-яё]*\s*обл[а-яё]*|ростовск[а-яё]*\s*обл[а-яё]*|красноярск[а-яё]*\s*кра[яй]|пермск[а-яё]*\s*кра[яй]|краснодарск[а-яё]*\s*кра[яй]|алтайск[а-яё]*\s*кра[яй]|ставропольск[а-яё]*\s*кра[яй]|хабаровск[а-яё]*\s*кра[яй]|приморск[а-яё]*\s*кра[яй]|забайкальск[а-яё]*\s*кра[яй]|камчатск[а-яё]*\s*кра[яй])(\s|$)/i,

      // Страны
      /(^|\s)(росси[яи]|рф|украин[аы]|белорусси[яи]|казахстан[а]?|сша|америк[аи]|германи[яи]|франци[яи]|итали[яи]|испани[яи]|кита[яй]|япони[яи]|инди[яи]|бразили[яи]|канад[аы]|австрали[яи]|швейцари[яи]|швеци[яи]|норвеги[яи]|финлянди[яи]|польш[аи]|чехи[яи]|словаки[яи]|венгри[яи]|румыни[яи]|болгари[яи]|греци[яи]|турци[яи])(\s|$)/i,
    ];

    return keywordsData.filter((item) => {
      const phrase = item.phrase.toLowerCase();
      // Проверяем, содержит ли фраза любое гео-слово как отдельное слово
      return !geoPatterns.some((pattern) => {
        // Ищем полное совпадение слова (с границами)
